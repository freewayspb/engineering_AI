...
- FR-OCR-01A: Обработка машиночитаемых PDF документов.  
   Функциональность: извлечение текста и структуры из машиночитаемых PDF без OCR (прямое чтение текстового слоя). Поддерживаемые элементы: текст, таблицы, изображения, векторная графика, метаданные документа. Алгоритм: парсинг PDF структуры, извлечение текстового контента, сохранение форматирования (шрифты, размеры, позиции), извлечение таблиц с сохранением структуры, обработка встроенных изображений. Результат: структурированный текст с метаданными форматирования, таблицы в структурированном виде, ссылки на изображения, метаданные документа (title, author, creationDate, etc.).
   Acceptance:
   - Корректно извлекается текст из машиночитаемых PDF; сохраняется форматирование и структура.
   - Таблицы извлекаются с сохранением структуры (строки/столбцы/объединения ячеек).
   - Метаданные документа извлекаются полностью; встроенные изображения обрабатываются.
   - Обработка работает для стандартных PDF без ошибок; время обработки ≤ 5 сек на документ.
   - Результаты сохраняются в структурированном виде для дальнейшей обработки (FR-OCR-04).
...
- FR-OCR-01C: Обработка изображений (JPG, PNG, TIFF).  
   Функциональность: OCR обработка изображений в форматах JPG, PNG, TIFF с предобработкой. Поддерживаемые форматы: JPG (JPEG), PNG, TIFF (включая многостраничные), BMP. Предобработка: автоматическое определение ориентации, коррекция перспективы, улучшение контраста, адаптивная бинаризация, удаление фонового шума. OCR: распознавание текста с поддержкой русского языка, детект таблиц и структур, обработка рукописного текста (опционально). Специальная обработка: детект печатей и штампов, обработка технических чертежей, распознавание формул и символов. Результат: извлеченный текст с координатами, таблицы, детектированные печати/штампы, метаданные качества.
   Acceptance:
   - Поддерживаются все форматы: JPG, PNG, TIFF (включая многостраничные), BMP.
   - Предобработка корректно улучшает качество изображений; автоматическая коррекция ориентации.
   - OCR работает с русским языком; качество CER ≤ 5%, WER ≤ 10% для печатного текста.
   - Детект таблиц, печатей, штампов работает корректно; координаты элементов точны.
   - Обработка рукописного текста (опционально) с приемлемым качеством; время обработки ≤ 20 сек на изображение.
...
- FR-OCR-03: Поддержка русского языка в документах.  
   Функциональность: специализированная поддержка русского языка во всех типах OCR обработки. Языковые модели: обученные модели для русского языка, поддержка кириллицы, специальных символов и знаков препинания. Словари и коррекция: встроенные словари русского языка для постобработки, коррекция типичных OCR ошибок, контекстная коррекция. Техническая терминология: специализированные словари для технических терминов, ГОСТов, нормативных документов, сокращений. Форматирование: поддержка русских стандартов форматирования документов, обработка русских дат, чисел, единиц измерения. Результат: корректно распознанный русский текст с минимальными ошибками, сохранение форматирования, метаданные качества распознавания.
   Acceptance:
   - Русский язык поддерживается во всех типах OCR; кириллица распознается корректно.
   - Словари и коррекция работают; типичные OCR ошибки исправляются автоматически.
   - Техническая терминология распознается с использованием специализированных словарей.
   - Русские стандарты форматирования поддерживаются; даты, числа, единицы измерения корректны.
   - Качество распознавания русского текста: CER ≤ 3%, WER ≤ 8%; время обработки не увеличивается.
- FR-OCR-04: Структурирование извлеченных данных для базы знаний.  
   Функциональность: преобразование результатов OCR в структурированные данные для базы знаний системы. Типы структурирования: текст с разметкой (заголовки, абзацы, списки), таблицы в структурированном виде (JSON/CSV), метаданные документа, координаты элементов. Алгоритм: анализ структуры документа, выделение логических блоков, определение типов контента, создание иерархической структуры. Результат: JSON структура с полями text, tables, metadata, coordinates, confidence, documentStructure. Интеграция: совместимость с FR-LEARN-02 (обучение системы), FR-PROJECT-03 (извлечение в контексте проектов), FR-DAT-02 (хранение метаданных).
   Acceptance:
   - Извлеченные данные структурируются в JSON формат; сохраняется иерархия документа.
   - Таблицы преобразуются в структурированный вид; координаты элементов сохраняются.
   - Метаданные документа извлекаются и структурируются; confidence scores включаются.
   - Структурированные данные совместимы с системой обучения и проектной работой.
   - Время структурирования ≤ 10 сек на документ; размер выходных данных оптимизирован.
- FR-OCR-05: Парсинг штампов документов (titleBlock).  
   Функциональность: извлечение полей штампа из PDF документов (как машиночитаемых, так и сканированных). Поля штампа: title (наименование), cipher (шифр), sheetNumber (номер листа), documentName (наименование документа), sheetFormat (формат листа), designStage (стадия проектирования), organization (организация), documentDate (дата документа), revisionNumber (номер изменения). Алгоритм: детект области штампа в документе, распознавание текста в области штампа, парсинг полей по шаблонам, валидация корректности извлеченных данных. Результат: структурированные данные штампа с полями и confidence scores, метаданные качества распознавания, ссылки на исходные координаты в документе.
   Acceptance:
   - Детект области штампа работает корректно; распознаются различные форматы штампов.
   - Все поля штампа извлекаются: title, cipher, sheetNumber, documentName, sheetFormat, designStage, organization, documentDate, revisionNumber.
   - Парсинг полей работает по шаблонам; валидация корректности данных выполняется.
   - Результаты содержат confidence scores и координаты; метаданные качества сохраняются.
   - Интеграция с FR-OCR-01A/01B/01C работает; результаты совместимы с FR-STAMP-01..03.
- FR-OCR-06: Ветвление пайплайна обработки PDF по типам.  
   Функциональность: автоматическое определение типа PDF документа и выбор соответствующего пайплайна обработки. Типы PDF: machine-readable (машиночитаемый с текстовым слоем), scan-image (сканированное изображение без текстового слоя), mixed (смешанный тип). Алгоритм детекции: анализ PDF структуры, проверка наличия текстового слоя, анализ качества изображений, определение необходимости OCR. Ветвление: для machine-readable → FR-OCR-01A, для scan-image → FR-OCR-01B с предобработкой, для mixed → комбинированный подход. Предобработка для сканов: выравнивание (deskew), шумоподавление (denoise), бинаризация, улучшение контраста.
   Acceptance:
   - Автоматическое определение типа PDF работает корректно; детекция machine-readable vs scan-image точна.
   - Ветвление пайплайна выполняется автоматически; выбирается соответствующий алгоритм обработки.
   - Предобработка для сканов применяется: deskew, denoise, binarization работают корректно.
   - Комбинированный подход для mixed PDF работает; результаты объединяются корректно.
   - Время детекции типа ≤ 2 сек; общее время обработки оптимизировано для каждого типа.
...
- FR-EXCEL-01: Создание структурированных отчетов в Excel по запросам.  
   Типы отчетов: сводные отчеты по проектам (аналогично "СВОД РД"), детальные отчеты по разделам ("СО РД", "КП", "Расчет 2"), сравнительные отчеты между проектами, бюджетные отчеты с детализацией по позициям.    Специализированные типы отчетов на основе живых юз кейсов:
   - Сводная спецификация материалов и оборудования: структура 8 столбцов, группировка по категориям (Оборудование/Материалы), извлечение из штампов документов и табличных данных спецификаций;
   - Унификация спецификации: расширенная структура 20 столбцов с проектной информацией, интеграция с метаданными проекта для стандартизации позиций;
   - Бюджетирование проекта: полная финансовая структура 27 столбцов, интеграция ценовой информации и договоров поставщиков, детализация по позициям.
   Пошаговые алгоритмы формирования отчетов на основе живых юз кейсов:
   - Алгоритм 1.1 "Сводная спецификация": (1) извлечь штамп документа (реквизиты), (2) найти таблицы спецификаций в PDF, (3) распознать заголовки таблиц, (4) извлечь данные по строкам (наименование, единица, количество), (5) классифицировать позиции по категориям (Оборудование/Материалы), (6) создать Excel с 8 столбцами, (7) сгруппировать по категориям;
   - Алгоритм 1.2 "Унификация спецификации": (1) взять результат алгоритма 1.1, (2) добавить проектные метаданные (шифр РД, стадия проектирования), (3) стандартизировать наименования по корпоративным справочникам, (4) унифицировать единицы измерения, (5) добавить технические характеристики, (6) создать Excel с 20 столбцами, (7) валидировать соответствие стандартам;
   - Алгоритм 1.3 "Бюджетирование проекта": (1) взять результат алгоритма 1.2, (2) интегрировать ценовую информацию из базы данных проекта, (3) привязать цены к позициям спецификации, (4) рассчитать стоимость по позициям (количество × цена), (5) добавить договорные условия поставщиков, (6) сформировать итоговые суммы по категориям, (7) создать Excel с 27 столбцами, (8) добавить финансовую аналитику.
   Структура отчета: заголовок с проектной информацией (projectName, rdCipher, designStage), таблицы с извлеченными данными (наименования, шифры, технические характеристики, единицы измерения, количества, цены, стоимости), итоговые суммы и агрегаты, ссылки на исходные документы. Многолистовые книги: поддержка нескольких листов в одной книге с перекрестными ссылками и формулами между листами. Источники данных: извлеченные titleBlock, tables, estimate из документов проекта (FR-PROJECT-03), метаданные проекта (FR-PROJECT-01), данные аналогов (FR-PROJECT-02). Автозаполнение: шапка отчета из projectName/rdCipher/designStage, формулы расчета итогов, ссылки между листами.
   Acceptance:
   - Формируются все типы отчетов: сводные, детальные, сравнительные, бюджетные с правильной структурой.
   - Специализированные отчеты создаются согласно живых юз кейсов: сводная спецификация (8 столбцов), унификация (20 столбцов), бюджетирование (27 столбцов).
   - Пошаговые алгоритмы выполняются корректно: алгоритм 1.1 (7 шагов), алгоритм 1.2 (7 шагов), алгоритм 1.3 (8 шагов).
   - Итерационный процесс работает: результат алгоритма 1.1 используется как вход для алгоритма 1.2, результат 1.2 - для алгоритма 1.3.
   - Структура отчета включает заголовок проекта, таблицы данных, итоги, ссылки на источники.
   - Многолистовые книги работают корректно; перекрестные ссылки и формулы между листами функционируют.
   - Данные берутся из извлеченных артефактов проекта; автозаполнение шапки и формул работает.
   - Отчеты совместимы с шаблонами подразделений (ПТО, сметный отдел); валидация структуры проходит.  
- FR-EXCEL-02: Форматирование таблиц и диаграмм в Excel.  
   Функциональность: автоматическое форматирование таблиц в Excel отчетах на основе корпоративных стандартов и типов данных. Форматирование таблиц: заголовки таблиц (жирный шрифт, выравнивание по центру, цвет фона), числовые данные (формат чисел с разделителями тысяч, выравнивание по правому краю), текстовые данные (выравнивание по левому краю), даты (стандартный формат даты), итоговые строки (жирный шрифт, цвет фона, границы). Стили и цвета: корпоративная цветовая схема (синий для заголовков, серый для итогов, белый для данных), границы таблиц (тонкие линии), чередование цветов строк для читаемости. Автоматическое форматирование: применение стилей на основе типов данных, условное форматирование для выделения важных значений, автоширина колонок, автообертка текста. Стандарты: соответствие корпоративным шаблонам ПТО и сметного отдела, единообразие форматирования между листами.
   Acceptance:
   - Заголовки таблиц форматируются корректно: жирный шрифт, центрирование, корпоративные цвета.
   - Числовые данные имеют правильное форматирование: разделители тысяч, выравнивание по правому краю.
   - Текстовые данные и даты форматируются согласно стандартам; итоговые строки выделяются.
   - Корпоративная цветовая схема применяется; границы и чередование цветов работают корректно.
   - Автоматическое форматирование работает; условное форматирование выделяет важные значения; автоширина колонок функционирует.  
- FR-EXCEL-03: Экспорт данных в Excel формат.  
   Функциональность: простой экспорт данных из системы в Excel форматы. Поддерживаемые форматы: XLSX (основной современный формат), XLS (совместимость со старыми версиями), CSV (для интеграций). Типы экспортируемых данных: результаты запросов пользователей, извлеченные таблицы из документов, метаданные проектов, данные аналогов, отчеты по проектам. Алгоритм экспорта: получение данных из базы знаний, преобразование в табличный формат, сохранение в выбранном формате. Настройки экспорта: выбор формата файла, имя файла, кодировка (для CSV). Результат: файл в выбранном формате с экспортированными данными, сохраненный в указанном расположении.
   Acceptance:
   - Экспорт работает для всех поддерживаемых форматов: XLSX, XLS, CSV.
   - Экспортируются все типы данных: результаты запросов, таблицы, метаданные, отчеты.
   - Настройки экспорта работают: выбор формата, имя файла, кодировка (CSV).
   - Экспортированные файлы корректно открываются в соответствующих приложениях.
   - Экспорт выполняется без ошибок; время экспорта ≤ 30 сек для стандартных объемов данных.  
- FR-EXCEL-04: Валидация корректности Excel отчетов.  
   Функциональность: проверка корректности созданных Excel отчетов на соответствие стандартам и шаблонам. Типы валидации: структура отчета (наличие обязательных листов, правильные названия листов), форматирование (соответствие корпоративным стилям, правильное форматирование заголовков и данных), данные (корректность типов данных, отсутствие пустых обязательных полей, правильность формул), ссылки (работоспособность перекрестных ссылок между листами, корректность формул). Критерии валидации: соответствие шаблонам подразделений (ПТО, сметный отдел), правильность структуры таблиц, корректность числовых значений и расчетов. Результат валидации: отчет о найденных ошибках с указанием листа, ячейки и типа ошибки, рекомендации по исправлению, общий статус валидации (успешно/с предупреждениями/с ошибками).
   Acceptance:
   - Валидация структуры работает: проверяются обязательные листы, названия, организация данных.
   - Валидация форматирования работает: проверяется соответствие корпоративным стилям и стандартам.
   - Валидация данных работает: проверяются типы данных, обязательные поля, корректность формул.
   - Валидация ссылок работает: проверяются перекрестные ссылки и формулы между листами.
   - Отчет о валидации содержит детальную информацию об ошибках и рекомендации; статус валидации определяется корректно.
- FR-EXCEL-05: Генерация отчетов по подразделениям (ПТО и сметный отдел).  
   Функциональность: создание специализированных отчетов для ПТО и сметного отдела с использованием утвержденных шаблонов и строгим соблюдением структуры и форматирования. Типы отчетов: отчеты ПТО (ведомости объемов, комплекты РД, технические спецификации), отчеты сметного отдела (сметы, калькуляции, ведомости ресурсов). Шаблоны: использование утвержденных шаблонов подразделений, строгое соблюдение структуры листов, названий, стилей, областей печати. Валидация: проверка соответствия активной версии шаблона, валидация типов данных и форматов, проверка обязательных полей, валидация формул и настроек печати. Результат: отчеты, полностью соответствующие корпоративным стандартам подразделений, с отчетом о соответствии шаблону.
   Acceptance:
   - Генерируются специализированные отчеты для ПТО и сметного отдела с правильной структурой.
   - Используются утвержденные шаблоны подразделений; структура и форматирование соблюдаются строго.
   - Валидация соответствия шаблону работает; проверяются все аспекты: листы, названия, стили, области печати.
   - Обязательные поля заполняются корректно; формулы и настройки печати валидируются.
   - Отчеты полностью соответствуют корпоративным стандартам; генерируется отчет о соответствии.
- FR-EXCEL-06: Многолистовые Excel-книги с умными связями.
  Контекст: Генерация проектных Excel-структур для спецификаций и смет.
  Основные возможности:
  - Создает типовые и проектные листы по пресетам операций.
  - Строит перекрестные ссылки и именованные диапазоны автоматически.
  - Поддерживает адаптивную структуру при изменении объема данных.
  Обучаемость и знания: Запоминает структуры книг и связи по типам проектов в workbookStructure.json.
  Acceptance:
  - Книга для сценария 1.1–1.3 создается ≤ 30 сек с корректными листами.
  - Ссылки между листами проходят валидацию без циклов и битых диапазонов.
  - Структура книги сохраняется в артефакте с версией и описанием проекта.
  Связи: BR-018, UC-05, FR-FORMULA-01, FR-EXCEL-TPL-01, FR-LOG-02, FR-KNOW-01.
- FR-EXCEL-07: Автозаполнение заголовков Excel из контекста проекта.
  Контекст: Генерация отчетов по шаблонам подразделений.
  Основные возможности:
  - Собирает значения из titleBlock, project metadata и профиля пользователя.
  - Применяет шаблон headerTemplates.json и проверяет обязательные поля.
  - Сохраняет метаданные заполнения: источник, confidence и время обновления.
  Обучаемость и знания: Накопление шаблонов заголовков по типам отчетов с версионированием.
  Acceptance:
  - Контрольный отчет заполняется ≤ 5 сек; обязательные поля проходят валидацию без ручного ввода.
  - headerTemplates.json содержит структуру и history последнего обновления шаблона.
  - При confidence < 0.7 создается рекомендация на уточнение и отметка в отчете проблем.
  Связи: FR-PROJECT-01, FR-OCR-05, FR-EXCEL-TPL-01, FR-KNOW-01, FR-LOG-02, FR-LEARN-03.
- FR-EXCEL-08: Бюджетирование проектов с ценовой информацией.
  Контекст: Финансовые отчеты для сценариев 1.1–1.3.
  Основные возможности:
  - Объединяет таблицы спецификаций с актуальными ценами и договорами.
  - Рассчитывает стоимости по позициям, категориям и итоги проекта.
  - Фиксирует версию используемых цен и источников в metadata.
  Acceptance:
  - Шаблон на 27 столбцов заполняется автоматически и проходит проверку FR-EXCEL-TPL-02..04.
  - Стоимость позиций совпадает с контрольным расчетом при допустимом отклонении ≤ 1%.
  - Источник цен и дата актуализации сохраняются для каждой строки и в сводном листе.
  Связи: FR-TABLE-01, FR-PROJECT-01, FR-EXCEL-06, FR-LOG-02.
- FR-EXCEL-TPL-01: Реестр шаблонов Excel с обучением.
  Контекст: Централизованное управление шаблонами отчетов.
  Основные возможности:
  - Хранит шаблоны с версиями, владельцами, статусами и областью применения.
  - Ведет аудит изменений и контроль доступа для подразделений.
  - Анализирует структуру шаблонов и предлагает обновления на основе использования.
  Обучаемость и знания: Накопление learningMetadata и рекомендаций по улучшению шаблонов.
  Acceptance:
  - CRUD API возвращает templateId, version, status, owner и learningMetadata.
  - История изменений фиксируется с author, timestamp и diff JSON для каждой версии.
  - Система предлагает новую версию при изменении структуры и логирует рекомендацию.
  Связи: FR-EXCEL-05..07, FR-LEARN-03, FR-KNOW-01, FR-ACCESS-01, FR-LOG-02.
- FR-EXCEL-TPL-02: Проверка соответствия активному шаблону.
  Контекст: Валидация отчетов перед передачей в подразделение.
  Основные возможности:
  - Сверяет набор листов, именованные диапазоны и стили с текущей версией шаблона.
  - Проверяет формулы, области печати, защиту и макросы на предмет отклонений.
  - Накопляет статистику ошибок для автоисправлений и обучения.
  Acceptance:
  - Контрольный отчет проходит проверку ≤ 10 сек; нарушения классифицируются как Critical/Warning/Info.
  - validationReport.json содержит лист, правило, рекомендацию и уровень критичности.
  - При обновлении шаблона правила проверки синхронизируются автоматически из реестра.
  Связи: FR-EXCEL-TPL-01, FR-EXCEL-06, FR-FORMULA-01, FR-LOG-02, FR-LEARN-04.
- FR-EXCEL-TPL-03: Контроль типов данных и обязательных полей.
  Контекст: Проверка содержимого Excel отчетов.
  Основные возможности:
  - Валидирует числовые, текстовые и проектные форматы по правилам шаблона.
  - Проверяет заполненность обязательных столбцов и диапазоны значений.
  - Предлагает автоисправления и обновляет статистику ошибок.
  Acceptance:
  - Тестовый отчет выявляет пустые/неверные поля и формирует fixSuggestions.json.
  - Нарушения включают ссылку на лист/ячейку, ожидаемый формат и рекомендацию.
  - Исправления пользователя повышают confidence правил в learningMetadata.
  Связи: FR-EXCEL-TPL-01, FR-EXCEL-TPL-02, FR-KNOW-01, FR-LEARN-04.
- FR-EXCEL-TPL-04: Комплексный отчет о соответствии шаблону.
  Контекст: Сводный результат проверок Excel-отчета.
  Основные возможности:
  - Агрегирует выводы FR-EXCEL-TPL-02 и FR-EXCEL-TPL-03 в единый отчет.
  - Рассчитывает метрики времени, количество нарушений и уровни критичности.
  - Формирует рекомендации и экспортирует сводку в PDF/XLSX по запросу.
  Acceptance:
  - consolidatedReport.json содержит секции structure/data/formula/print с статусом и советом.
  - Пересчет занимает ≤ 5 сек для файла 10 МБ; время и количество нарушений логируются.
  - Пользователь может выгрузить summary.xlsx или summary.pdf с текущими результатами.
  Связи: FR-EXCEL-TPL-02, FR-EXCEL-TPL-03, FR-FORMULA-02, FR-LOG-02, FR-LEARN-04.
...
- FR-NORMS-01: Загрузка и изучение нормативных документов (ГОСТы).  
   Функциональность: загрузка и обработка нормативных документов для создания базы знаний по нормативам. Типы документов: ГОСТ, СП (Своды правил), СНиП (Строительные нормы и правила), ТУ (Технические условия), РД (Руководящие документы). Форматы документов: PDF (машиночитаемые и сканированные), DOCX (нормативные документы), HTML (онлайн версии). Процесс обработки: загрузка документа, определение типа и формата, извлечение текста и структуры, парсинг метаданных (название, номер, дата утверждения, редакция), извлечение требований и правил, структурирование для базы знаний. Результат: структурированные данные нормативного документа с метаданными, извлеченными требованиями и ссылками на исходный документ.
   Acceptance:
   - Поддерживаются все типы нормативных документов: ГОСТ, СП, СНиП, ТУ, РД.
   - Поддерживаются все форматы: PDF (машиночитаемые/сканированные), DOCX, HTML.
   - Метаданные документов извлекаются корректно: название, номер, дата утверждения, редакция.
   - Требования и правила извлекаются и структурируются для базы знаний.
   - Ссылки на исходные документы сохраняются; интеграция с FR-NORMS-02 работает.
- FR-NORMS-02: Создание базы знаний по нормативам.  
   Функциональность: создание структурированной базы знаний на основе обработанных нормативных документов. Структура базы знаний: категории нормативов (строительство, проектирование, материалы, оборудование), иерархия требований, связи между нормативами, версионирование требований. Типы знаний: конкретные требования (числовые значения, диапазоны, условия), процедурные правила (последовательность действий, условия применения), ссылочные данные (таблицы, формулы, коэффициенты). Алгоритм: группировка требований по категориям, создание связей между нормативами, индексация для поиска, версионирование при обновлениях. Результат: структурированная база знаний с возможностью поиска, фильтрации и извлечения релевантных требований.
   Acceptance:
   - База знаний создается с правильной структурой: категории, иерархия, связи, версионирование.
   - Типы знаний поддерживаются: требования, правила, ссылочные данные.
   - Группировка и связи между нормативами работают корректно.
   - Индексация для поиска функционирует; фильтрация работает по категориям.
   - Версионирование при обновлениях работает; интеграция с FR-NORMS-03..04 обеспечена.
- FR-NORMS-03: Валидация данных по нормативным требованиям.  
   Функциональность: проверка соответствия проектных данных нормативным требованиям из базы знаний. Типы валидации: соответствие числовых значений (размеры, нагрузки, коэффициенты), соблюдение процедурных правил (последовательность проектирования, условия применения), проверка ссылочных данных (использование правильных таблиц, формул). Алгоритм: извлечение требований из базы знаний по типу данных, сравнение с проектными данными, определение соответствия/несоответствия, генерация отчета о валидации. Результат: отчет о соответствии нормативным требованиям с указанием нарушений, рекомендациями по исправлению и ссылками на конкретные пункты нормативов.
   Acceptance:
   - Валидация числовых значений работает: проверяются размеры, нагрузки, коэффициенты.
   - Валидация процедурных правил работает: проверяется последовательность, условия применения.
   - Валидация ссылочных данных работает: проверяются таблицы, формулы, коэффициенты.
   - Отчет о валидации содержит нарушения, рекомендации, ссылки на пункты нормативов.
   - Интеграция с базой знаний работает; валидация выполняется автоматически.
- FR-NORMS-04: Поиск по нормативной базе.  
   Функциональность: поиск релевантных нормативных требований в базе знаний по различным критериям. Типы поиска: текстовый поиск по содержанию требований, поиск по категориям нормативов, поиск по числовым значениям и диапазонам, поиск по датам утверждения и редакциям. Критерии поиска: ключевые слова, тип норматива, категория требований, числовые диапазоны, даты. Алгоритм: индексация базы знаний, семантический поиск по содержанию, фильтрация по критериям, ранжирование результатов по релевантности. Результат: список релевантных нормативных требований с указанием источника, степени соответствия запросу и ссылками на полный текст.
   Acceptance:
   - Текстовый поиск работает: поиск по содержанию требований, ключевым словам.
   - Поиск по категориям работает: фильтрация по типу норматива, категории требований.
   - Поиск по числовым значениям работает: поиск по диапазонам, точным значениям.
   - Поиск по датам работает: фильтрация по датам утверждения, редакциям.
   - Результаты ранжируются по релевантности; ссылки на источники корректны.
- FR-NORM-05: Парсинг метаданных DOCX нормативных документов и выжимки по запросу.  
   Функциональность: специализированная обработка DOCX нормативных документов с извлечением метаданных и созданием выжимок по запросам. Метаданные DOCX: название документа, номер норматива, дата утверждения, редакция, организация-разработчик, область применения. Парсинг структуры: извлечение заголовков разделов, нумерации пунктов, таблиц и приложений. Выжимки по запросу: создание релевантных выдержек из документа на основе поискового запроса, сохранение контекста и ссылок на исходные пункты. Алгоритм: анализ структуры DOCX, извлечение метаданных из свойств документа, индексация содержания, генерация выжимок по запросам. Результат: структурированные метаданные DOCX документов и релевантные выжимки с сохранением ссылок на исходные пункты.
   Acceptance:
   - Метаданные DOCX извлекаются корректно: название, номер, дата утверждения, редакция, организация.
   - Структура документа парсится: заголовки, нумерация, таблицы, приложения.
   - Выжимки по запросу создаются корректно; контекст и ссылки сохраняются.
   - Индексация содержания работает; поиск по выжимкам функционирует.
   - Интеграция с FR-NORMS-01..04 работает; метаданные используются в базе знаний.
- FR-NORM-06: Экспорт выжимок в PDF со ссылками на источник.  
   Функциональность: экспорт созданных выжимок нормативных документов в PDF формат с сохранением ссылок на исходные источники. Формат PDF: структурированный документ с заголовками, текстом выжимок, ссылками на источники, библиографией. Ссылки на источники: точные ссылки на пункты нормативов, номера страниц, даты редакций, полные библиографические данные. Структура экспорта: заголовок документа, список выжимок с источниками, библиографический список, приложения. Результат: PDF документ с выжимками, содержащий все необходимые ссылки для воспроизводимости и проверки.
   Acceptance:
   - Экспорт в PDF работает корректно; структура документа сохраняется.
   - Ссылки на источники точны: пункты нормативов, страницы, редакции, библиография.
   - Структура экспорта правильная: заголовок, выжимки, источники, библиография.
   - PDF документ содержит все необходимые ссылки для воспроизводимости.
   - Интеграция с FR-NORM-05 работает; выжимки экспортируются корректно.
- FR-NORM-07: Версионирование и политика актуализации нормативных документов.  
   Функциональность: управление версиями нормативных документов и политика их актуализации в локальной системе. Версионирование: хранение номеров редакций и изменений, метаданные версий (код, номер, дата, редакция), отслеживание изменений между версиями. Политика актуализации: правила обновления нормативов при поступлении новых редакций, уведомления об изменениях, миграция данных между версиями. Локальные источники: работа с локальными копиями нормативов без доступа к интернету, синхронизация при наличии обновлений. Результат: система версионирования нормативов с возможностью отслеживания изменений и актуализации локальной базы знаний.
   Acceptance:
   - Версионирование работает: номера редакций, метаданные версий, отслеживание изменений.
   - Политика актуализации функционирует: правила обновления, уведомления, миграция данных.
   - Локальные источники поддерживаются; синхронизация работает при наличии обновлений.
   - Отслеживание изменений между версиями работает корректно.
   - Интеграция с FR-NORMS-01..04 работает; версионирование применяется ко всей базе знаний.
- FR-NORM-08: Хранение извлеченных требований как структурированных правил.  
   Функциональность: преобразование извлеченных нормативных требований в структурированные правила с условиями, диапазонами и ссылками на пункты. Структурированные правила: условия применения (если-то логика), числовые диапазоны и ограничения, ссылки на пункты нормативов, связи между правилами. Формат хранения: JSON структура с полями condition, range, clause_reference, rule_type, applicability. Алгоритм: анализ извлеченных требований, выделение условий и ограничений, создание структурированных правил, установление связей между правилами. Результат: база структурированных правил, пригодная для автоматической валидации и применения в проектной работе.
   Acceptance:
   - Структурированные правила создаются корректно: условия, диапазоны, ссылки на пункты.
   - JSON формат хранения работает; структура правил соответствует схеме.
   - Алгоритм анализа требований функционирует; условия и ограничения выделяются.
   - Связи между правилами устанавливаются корректно.
   - Интеграция с FR-NORMS-02..03 работает; правила используются для валидации.
- FR-NORM-09: Генерация отчетов о несоответствиях с точными ссылками на пункты.  
   Функциональность: создание детальных отчетов о несоответствиях проектных данных нормативным требованиям с точными ссылками на конкретные пункты нормативов. Структура отчета: список несоответствий с указанием типа нарушения, точные ссылки на пункты нормативов, описание требований, рекомендации по исправлению. Точные ссылки: номер норматива, пункт, подпункт, страница, редакция, дата утверждения. Классификация нарушений: критические (блокирующие), предупреждения (рекомендательные), информационные (для справки). Результат: структурированный отчет о несоответствиях с возможностью экспорта в различные форматы и интеграции с системой управления проектами.
   Acceptance:
   - Отчеты о несоответствиях генерируются корректно; структура отчета правильная.
   - Точные ссылки на пункты нормативов работают: номер, пункт, подпункт, страница, редакция.
   - Классификация нарушений функционирует: критические, предупреждения, информационные.
   - Рекомендации по исправлению содержат конкретные действия.
   - Экспорт в различные форматы работает; интеграция с системой управления проектами обеспечена.
...
- FR-DAT-01: Автоопределение типа документа (pdf/image/dwg/smeta_arp/smeta_gsfx/normative/unknown).  
   Функциональность: автоматическое определение типа загружаемого документа для выбора соответствующего пайплайна обработки. Типы документов: PDF (машиночитаемые и сканированные), изображения (JPG, PNG, TIFF, BMP), DWG (AutoCAD чертежи), сметы ARP (формат сметного ПО), сметы GSFX (формат сметного ПО), нормативные документы (ГОСТ, СП, СНиП), неизвестные форматы. Алгоритм детекции: анализ расширения файла, проверка MIME-типа, анализ заголовков файла, проверка содержимого для подтверждения типа. Результат детекции: определенный тип документа с уровнем уверенности, рекомендация пайплайна обработки, предупреждения о возможных проблемах. Интеграция: связь с соответствующими пайплайнами обработки (FR-OCR-*, FR-STAMP-*, FR-TABLE-*, FR-ESTIMATE-*).
   Acceptance:
   - Определяются все типы документов: PDF, изображения, DWG, сметы ARP/GSFX, нормативные, неизвестные.
   - Алгоритм детекции работает корректно: анализ расширения, MIME-типа, заголовков, содержимого.
   - Результат содержит тип документа, уровень уверенности, рекомендацию пайплайна.
   - Предупреждения о проблемах генерируются корректно.
   - Интеграция с пайплайнами обработки работает; правильный пайплайн выбирается автоматически.
- FR-DAT-02: Хранение метаданных (имя файла, MIME, язык, страницы, дата загрузки, проект).  
   Функциональность: централизованное хранение метаданных документов в структурированном формате. Структура метаданных: базовые данные (имя файла, MIME-тип, размер файла, дата загрузки), языковые данные (определенный язык документа, кодировка), структурные данные (количество страниц, размеры страниц, ориентация), проектные данные (projectId, projectName, designStage, rdCipher), технические данные (хеш файла, версия документа, статус обработки). Формат хранения: JSON структура с полями fileName, mimeType, fileSize, uploadDate, language, encoding, pageCount, pageSize, orientation, projectId, projectName, designStage, rdCipher, fileHash, documentVersion, processingStatus. Индексация: создание индексов для быстрого поиска по метаданным, поддержка фильтрации и сортировки. Результат: структурированные метаданные с возможностью быстрого поиска и фильтрации.
   Acceptance:
   - Все метаданные сохраняются корректно: базовые, языковые, структурные, проектные, технические данные.
   - JSON формат хранения работает; структура соответствует схеме.
   - Индексация функционирует; поиск и фильтрация работают быстро.
   - Связь с проектами работает; projectId корректно привязывается.
   - Статус обработки отслеживается; обновления метаданных работают корректно.
- FR-DAT-03: Логирование шагов обработки и обучения (pipeline).  
   Функциональность: детальное логирование всех этапов обработки документов и обучения системы. Типы логов: системные логи (загрузка файлов, ошибки системы, производительность), логи обработки (этапы пайплайна, результаты OCR, извлечение данных), логи обучения (обновления базы знаний, валидация качества), пользовательские логи (действия пользователей, запросы, результаты). Структура лога: timestamp (время события), level (уровень: DEBUG/INFO/WARNING/ERROR), code (код события), message (описание события), documentId (ID документа), inputPath (путь к входному файлу), outputPath (путь к результату), stage (этап обработки), details (дополнительные детали). Хранение: структурированные логи в JSON формате, ротация логов по размеру и времени, архивирование старых логов. Результат: полная трассировка обработки документов с возможностью отладки и анализа производительности.
   Acceptance:
   - Все типы логов записываются: системные, обработки, обучения, пользовательские.
   - Структура лога соответствует схеме: timestamp, level, code, message, documentId, inputPath, outputPath, stage, details.
   - Логирование работает на всех этапах пайплайна обработки.
   - Ротация и архивирование логов функционируют корректно.
   - Трассировка обработки документов полная; отладка и анализ производительности возможны.
- FR-DAT-04: Версионирование результатов обучения и базы знаний.  
   Функциональность: управление версиями результатов обучения системы и базы знаний с возможностью отката к предыдущим версиям. Объекты версионирования: результаты обучения (извлеченные знания, структурированные данные), база знаний (нормативные требования, проектные данные, метаданные), конфигурация системы (настройки обработки, шаблоны, правила). Структура версии: versionId (уникальный идентификатор версии), timestamp (время создания), description (описание изменений), author (автор изменений), changes (список изменений), rollbackData (данные для отката). Операции: создание новой версии при значительных изменениях, сравнение версий, откат к предыдущей версии, просмотр истории изменений. Результат: система версионирования с возможностью отслеживания изменений и восстановления предыдущих состояний.
   Acceptance:
   - Версионируются все объекты: результаты обучения, база знаний, конфигурация системы.
   - Структура версии содержит все необходимые поля: versionId, timestamp, description, author, changes, rollbackData.
   - Операции версионирования работают: создание, сравнение, откат, просмотр истории.
   - Откат к предыдущим версиям функционирует корректно.
   - История изменений сохраняется полностью; трассировка изменений работает.
- FR-DAT-05: Детекция типа PDF источника и маршрутизация в специализированный пайплайн.  
   Функциональность: определение типа PDF документа (машиночитаемый или сканированный) и направление в соответствующий пайплайн обработки. Типы PDF: mc_pdf (машиночитаемый PDF с текстовым слоем), scan_pdf (сканированный PDF как изображение), mixed_pdf (смешанный тип с текстом и изображениями). Алгоритм детекции: анализ наличия текстового слоя, проверка качества текста, анализ структуры документа, определение соотношения текста и изображений. Маршрутизация: mc_pdf → FR-OCR-01A (прямое извлечение текста), scan_pdf → FR-OCR-01B (OCR с предобработкой), mixed_pdf → комбинированный подход. Результат: определенный тип PDF с рекомендацией пайплайна и параметрами обработки.
   Acceptance:
   - Определяются все типы PDF: машиночитаемые, сканированные, смешанные.
   - Алгоритм детекции работает корректно: анализ текстового слоя, качества, структуры.
   - Маршрутизация в пайплайны работает: правильный пайплайн выбирается автоматически.
   - Параметры обработки определяются корректно для каждого типа.
   - Интеграция с FR-OCR-01A/01B работает; обработка оптимизирована для каждого типа.
- FR-DAT-06: Парсинг штампов DWG и извлечение табличного контента из текстовых слоев/блоков.  
   Функциональность: специализированная обработка DWG файлов с извлечением штампов и табличного контента. Парсинг штампов: извлечение полей штампа (title, cipher, sheetNumber, documentName, sheetFormat, designStage, organization, documentDate, revisionNumber), определение координат штампа, валидация корректности полей. Извлечение табличного контента: анализ текстовых слоев DWG, извлечение таблиц из блоков, сохранение структуры таблиц, определение типов данных. Алгоритм: парсинг DWG структуры, извлечение текстовых объектов, группировка в таблицы, валидация данных. Результат: структурированные данные штампа и таблиц DWG в формате, совместимом с общей системой метаданных.
   Acceptance:
   - Штампы DWG парсятся корректно: все поля извлекаются, координаты определяются.
   - Табличный контент извлекается: текстовые слои, блоки, структура таблиц.
   - Валидация данных работает; корректность полей проверяется.
   - Структура таблиц сохраняется; типы данных определяются правильно.
   - Интеграция с общей системой метаданных работает; формат совместим.
- FR-DAT-07: Парсинг смет (ARP/GSFX/XML/XLS) в единую модель Estimate с валидацией итогов/единиц и привязкой к шифру РД.  
   Функциональность: унифицированная обработка смет в различных форматах с приведением к единой модели данных. Форматы смет: ARP (формат сметного ПО), GSFX (формат сметного ПО), XML (стандартный XML), XLS (Excel формат). Единая модель Estimate: секции → позиции → ресурсы с полями (name, unit, qty, unitPrice, amount). Валидация: проверка итогов и промежуточных сумм, валидация единиц измерения, проверка обязательных полей, валидация диапазонов значений. Привязка к РД: связь позиций сметы с шифром рабочей документации, сохранение контекста проекта. Алгоритм: парсинг исходного формата, преобразование в единую модель, валидация данных, привязка к проекту. Результат: структурированные сметы в едином формате с валидацией и привязкой к проекту.
   Acceptance:
   - Все форматы смет поддерживаются: ARP, GSFX, XML, XLS.
   - Единая модель Estimate работает: секции → позиции → ресурсы с правильными полями.
   - Валидация функционирует: итоги, единицы, обязательные поля, диапазоны значений.
   - Привязка к шифру РД работает; контекст проекта сохраняется.
   - Интеграция с FR-ESTIMATE-* работает; единый формат используется во всей системе.
- FR-DAT-08: Сохранение полей штампа заголовка в DocumentMetadata.titleBlock с уверенностью по полям и аудит-логом.  
   Функциональность: структурированное сохранение извлеченных полей штампа документа с метриками качества и аудитом изменений. Структура titleBlock: поля штампа (title, cipher, sheetNumber, documentName, sheetFormat, designStage, organization, documentDate, revisionNumber), координаты полей, размеры полей. Уверенность по полям: confidence score для каждого поля (0-100%), источник извлечения (OCR, парсинг, ручной ввод), метрики качества. Аудит-лог: история изменений полей, автор изменений, время изменения, причина изменения, предыдущие значения. Интеграция: связь с DocumentMetadata, обновление при повторной обработке, версионирование изменений. Результат: полная трассировка полей штампа с метриками качества и историей изменений.
   Acceptance:
   - Все поля штампа сохраняются: title, cipher, sheetNumber, documentName, sheetFormat, designStage, organization, documentDate, revisionNumber.
   - Уверенность по полям работает: confidence score, источник извлечения, метрики качества.
   - Аудит-лог функционирует: история изменений, автор, время, причина, предыдущие значения.
   - Интеграция с DocumentMetadata работает; обновления при повторной обработке корректны.
   - Версионирование изменений работает; трассировка полей полная.
- FR-LOG-01: Единый формат лога обработки.  
   Функциональность: стандартизированный формат логирования всех операций обработки документов в системе. Структура лога: timestamp (время события в UTC), level (уровень логирования: DEBUG/INFO/WARNING/ERROR), code (уникальный код события), message (человекочитаемое описание), documentId (идентификатор документа), inputPath (путь к входному файлу), outputPath (путь к результату), stage (этап обработки), details (дополнительные детали в JSON). Применение: все операции системы используют единый формат, логи структурированы для машинного анализа, поддержка фильтрации и поиска. Результат: консистентные логи всех операций с возможностью анализа и отладки.
   Acceptance:
   - Единый формат лога применяется ко всем операциям: timestamp, level, code, message, documentId, inputPath, outputPath, stage, details.
   - Структура лога соответствует стандарту; все поля заполняются корректно.
   - Логи структурированы для машинного анализа; фильтрация и поиск работают.
   - Все операции системы используют единый формат логирования.
   - Анализ и отладка возможны; трассировка операций полная.
- FR-LOG-02: Сохранение артефактов обработки в предсказуемой структуре каталогов.  
   Функциональность: организация сохранения всех артефактов обработки документов в структурированной файловой системе. Типы артефактов: метаданные (JSON с описанием документа), titleBlock (извлеченные поля штампа), tables (табличные данные), estimate (сметные данные), logs (логи обработки). Структура каталогов: /artifacts/{projectId}/{documentId}/{timestamp}/{type}/, предсказуемые имена файлов, версионирование артефактов. Алгоритм: создание структуры каталогов при обработке, сохранение артефактов в соответствующие папки, создание индексов для быстрого поиска. Результат: организованная файловая структура с возможностью быстрого доступа к артефактам обработки.
   Acceptance:
   - Все типы артефактов сохраняются: метаданные, titleBlock, tables, estimate, logs.
   - Структура каталогов предсказуема: /artifacts/{projectId}/{documentId}/{timestamp}/{type}/.
   - Имена файлов стандартизированы; версионирование артефактов работает.
   - Индексы для поиска создаются; быстрый доступ к артефактам обеспечен.
   - Организация файловой структуры работает корректно; артефакты доступны.
- FR-ART-01: Формирование набора артефактов для демонстраций с ссылками для воспроизводимости.  
   Функциональность: создание демонстрационных наборов артефактов обработки с полной трассировкой для воспроизводимости результатов. Состав набора: исходные документы, промежуточные результаты обработки, финальные артефакты, логи обработки, метаданные. Форматы артефактов: JSON (структурированные данные), XLSX (таблицы и отчеты), PDF (документы), TXT (логи). Ссылки для воспроизводимости: точные ссылки на исходные файлы, параметры обработки, версии компонентов, конфигурация системы. Структура демо-набора: каталог с исходными данными, результаты обработки, документация по воспроизведению. Результат: готовый демонстрационный набор с возможностью полного воспроизведения результатов обработки.
   Acceptance:
   - Демонстрационные наборы создаются корректно: исходные документы, промежуточные результаты, финальные артефакты, логи, метаданные.
   - Все форматы артефактов поддерживаются: JSON, XLSX, PDF, TXT.
   - Ссылки для воспроизводимости точны: исходные файлы, параметры, версии, конфигурация.
   - Структура демо-набора правильная: каталог с данными, результаты, документация.
   - Воспроизведение результатов возможно; демонстрации работают корректно.

Связанные UC: UC-21, UC-23, UC-25
...
- FR-STAMP-01: Извлечение полей штампа документа с интеллектуальным обучением и адаптацией к проектной специфике.  
   Функциональность: извлечение полей штампа рабочей документации из PDF и DWG файлов с использованием специализированных алгоритмов распознавания и интеллектуального обучения системы. Поля штампа: title (наименование документа), cipher (шифр документа), sheetNumber (номер листа), documentName (наименование документа), sheetFormat (формат листа), designStage (стадия проектирования), organization (организация), documentDate (дата документа), revisionNumber (номер редакции). Алгоритм извлечения с обучаемостью: детекция области штампа в документе с использованием проектных шаблонов, распознавание текста в области штампа с применением проектных словарей, парсинг полей по адаптивным шаблонам, валидация извлеченных данных против проектных стандартов, интерактивное обучение при низкой уверенности. Поддержка форматов: PDF (машиночитаемые и сканированные), DWG (AutoCAD чертежи). Обучаемость системы: накопление шаблонов штампов по типам документов и проектам, запоминание типовых полей и их расположения, адаптация к проектной специфике через анализ истории штампов, интерактивное обучение при неоднозначности (вопросы учителю с запоминанием ответов), формирование правил извлечения на основе корректировок пользователя. Интеграция с системой обучения: сохранение извлеченных штампов в артефакте titleBlock.json (FR-LOG-02), накопление проектных словарей штампов (FR-KNOW-01), версионирование знаний о штампах (FR-LEARN-03), применение контекста проекта при извлечении (FR-PROJECT-03). Результат: структурированные данные полей штампа с метаданными (confidence score, источник извлечения, примененные правила, проектный контекст), координаты полей, метрики качества распознавания, рекомендации по улучшению извлечения. Обработка ошибок: некорректные поля помечаются с уровнем критичности, формируется детальный отчет о проблемных полях, сохраняется частичная информация при ошибках парсинга, логирование всех операций извлечения и обучения.  
   Acceptance:
   - Извлекаются все поля штампа: title, cipher, sheetNumber, documentName, sheetFormat, designStage, organization, documentDate, revisionNumber с применением проектных шаблонов и словарей.
   - Алгоритм извлечения работает для PDF и DWG форматов с адаптацией к проектной специфике.
   - Обучаемость системы функционирует: накапливаются шаблоны штампов, адаптация к проектам, интерактивное обучение при неоднозначности.
   - Интеграция с системой обучения работает: штампы сохраняются в titleBlock.json, накапливаются в проектных словарях, версионируются знания.
   - Детекция области штампа функционирует корректно с использованием проектных шаблонов.
   - Парсинг полей по адаптивным шаблонам работает; валидация данных выполняется против проектных стандартов.
   - Результат содержит структурированные данные с координатами, метриками качества, метаданными обучения и рекомендациями.
   - Обработка ошибок работает: некорректные поля помечаются, формируется отчет, сохраняется частичная информация, логируются операции.
- FR-STAMP-02: Сохранение уверенности по полям и происхождения в аудит-логе.  
   Функциональность: сохранение метрик качества распознавания для каждого поля штампа и ведение аудит-лога изменений. Уверенность по полям: confidence score для каждого поля (0-100%), источник извлечения (OCR, парсинг, ручной ввод), метрики качества распознавания. Аудит-лог: история изменений полей штампа, автор изменений, время изменения, причина изменения, предыдущие значения, источник изменений. Структура аудита: timestamp, fieldName, oldValue, newValue, confidence, source, author, reason, documentId. Интеграция: связь с DocumentMetadata.titleBlock, обновление при повторной обработке, версионирование изменений. Результат: полная трассировка изменений полей штампа с метриками качества и историей модификаций.
   Acceptance:
   - Уверенность по полям сохраняется: confidence score, источник извлечения, метрики качества.
   - Аудит-лог ведется корректно: история изменений, автор, время, причина, предыдущие значения.
   - Структура аудита соответствует схеме: timestamp, fieldName, oldValue, newValue, confidence, source, author, reason, documentId.
   - Интеграция с DocumentMetadata работает; обновления при повторной обработке корректны.
   - Версионирование изменений функционирует; трассировка полей полная.
- FR-STAMP-03: Валидация полноты и форматов полей штампа документа.  
   Функциональность: проверка корректности и полноты извлеченных полей штампа документа согласно стандартам рабочей документации. Валидация полноты: проверка наличия всех обязательных полей штампа, определение критичности отсутствующих полей, генерация предупреждений о неполноте. Валидация форматов: проверка соответствия форматов полей стандартам (даты в правильном формате, номера листов в числовом формате, шифры в соответствии с номенклатурой), валидация диапазонов значений. Критерии валидации: обязательные поля (title, cipher, sheetNumber), рекомендуемые поля (designStage, organization, documentDate), опциональные поля (revisionNumber, sheetFormat). Результат: отчет о валидации с указанием ошибок, предупреждений и рекомендаций по исправлению.
   Acceptance:
   - Валидация полноты работает: проверяются обязательные поля, определяется критичность отсутствующих.
   - Валидация форматов функционирует: проверяется соответствие стандартам, диапазоны значений.
   - Критерии валидации применяются корректно: обязательные, рекомендуемые, опциональные поля.
   - Отчет о валидации содержит ошибки, предупреждения, рекомендации по исправлению.
   - Интеграция с FR-STAMP-01..02 работает; валидация выполняется автоматически.
...
- FR-ESTIMATE-01: Унифицированная модель данных для сметных документов. Система должна предоставлять единую модель Estimate для представления сметных документов в структурированном виде независимо от исходного формата (ARP, GSFX, XML, XLS). Модель должна обеспечивать иерархию: разделы → позиции → ресурсы с полным набором атрибутов (наименование, единица измерения, количество, единичная цена, сумма) для каждого уровня. Обязательные поля: наименование, единица измерения, количество для позиций; код, наименование для разделов; наименование, номер для заголовка. Валидация: количество > 0, единичная цена ≥ 0, сумма = количество × единичная цена, соответствие единиц измерения справочнику. Обработка ошибок с уровнями критичности и формированием отчетов о проблемных полях.  
...
#### 6.1.1 Основные функции системы
- **AC-FUNC-01**: Система успешно обучается на корпоративных документах (≥90% документов обработаны без критических ошибок)
- **AC-FUNC-02**: Текстовые запросы обрабатываются корректно (≥90% запросов возвращают релевантные результаты)
- **AC-FUNC-03**: Проектно-ориентированная работа функционирует (100% данных привязаны к проектам)
- **AC-FUNC-04**: OCR распознавание работает для всех поддерживаемых форматов (≥95% точность для качественных документов)
- **AC-FUNC-05**: Excel отчеты генерируются по шаблонам подразделений (≥95% корректность форматирования)
- **AC-FUNC-06**: Нормативная база интегрирована и используется для валидации (≥90% покрытие приоритетных ГОСТов)

#### 6.1.2 Производительность
- **AC-PERF-01**: Время обработки текстового запроса — ≤5 секунд
- **AC-PERF-02**: Время обучения системы на документе — ≤30 секунд
- **AC-PERF-03**: Время формирования Excel отчета — ≤10 секунд
- **AC-PERF-04**: Время OCR обработки документа — ≤60 секунд
- **AC-PERF-05**: Время поиска аналогов по проектам — ≤3 секунды

#### 6.1.3 Качество данных
- **AC-QUAL-01**: Точность извлечения штампов документов — ≥97%
- **AC-QUAL-02**: Точность парсинга смет — ≥95% полей корректно извлечены
- **AC-QUAL-03**: Точность классификации документов — ≥90% (F1-score)
- **AC-QUAL-04**: Качество поиска аналогов — ≥80% релевантных результатов

### 6.2 Нефункциональные критерии

#### 6.2.1 Безопасность
- **AC-SEC-01**: Аутентификация и авторизация работают корректно
- **AC-SEC-02**: Данные шифруются в покое и при передаче
- **AC-SEC-03**: Аудит действий пользователей ведется полностью
- **AC-SEC-04**: Разграничение доступа по ролям функционирует
- **AC-SEC-05**: Система работает в изолированном окружении (air-gapped)

#### 6.2.2 Надежность
- **AC-REL-01**: Система обрабатывает ошибки входных данных gracefully
- **AC-REL-02**: Результаты воспроизводимы на одинаковых входах
- **AC-REL-03**: Отдельные модули не блокируют работу других
- **AC-REL-04**: Резервное копирование и восстановление функционируют
- **AC-REL-05**: Система работает 24/7 без критических сбоев

#### 6.2.3 Масштабируемость
- **AC-SCAL-01**: Система поддерживает горизонтальное масштабирование
- **AC-SCAL-02**: Модули могут обновляться независимо
- **AC-SCAL-03**: Система обрабатывает увеличение нагрузки линейно
- **AC-SCAL-04**: База знаний масштабируется с ростом документов

### 6.3 Критерии развертывания

#### 6.3.1 Инфраструктура
- **AC-DEPLOY-01**: Система развертывается на минимальных требованиях
- **AC-DEPLOY-02**: Docker-контейнеры запускаются корректно
- **AC-DEPLOY-03**: Конфигурация системы настраивается через файлы
- **AC-DEPLOY-04**: Мониторинг и логирование работают из коробки
- **AC-DEPLOY-05**: Обновления системы выполняются без потери данных

#### 6.3.2 Интеграции
- **AC-INT-01**: API для внешних систем функционирует
- **AC-INT-02**: Экспорт данных в стандартных форматах работает
- **AC-INT-03**: Импорт документов из внешних источников функционирует
- **AC-INT-04**: Интеграция с корпоративными системами возможна

### 6.4 Критерии пользовательского опыта

#### 6.4.1 Интерфейс
- **AC-UX-01**: Web-интерфейс интуитивно понятен пользователям
- **AC-UX-02**: Загрузка документов выполняется без ошибок
- **AC-UX-03**: Текстовые запросы вводятся и обрабатываются удобно
- **AC-UX-04**: Результаты отображаются в понятном формате
- **AC-UX-05**: Система поддерживает русский язык полностью

#### 6.4.2 Документация
- **AC-DOC-01**: Руководство пользователя доступно и понятно
- **AC-DOC-02**: Техническая документация для администраторов полная
- **AC-DOC-03**: API документация актуальна и подробна
- **AC-DOC-04**: Примеры использования предоставлены
...
3. **Тестирование безопасности**: проверка защиты данных
4. **Пользовательское тестирование**: проверка удобства использования
