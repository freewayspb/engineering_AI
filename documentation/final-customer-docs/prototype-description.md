...
### 3.1 Фактические модели (пилотный стенд, офлайн)
- **Назначение**: минимальный оффлайн-стек на пилотном железе без исходящего трафика; подобран по ресурсоёмкости и устойчивости к смешанным форматам.
- **LLM / структурированные данные**: `deepseek-r1` — ответы по JSON/конвертированным ARP/GSFX/DXF/DWG/RTF/XLSX; язык ответа задаётся системной инструкцией. Выбор: устойчивость при умеренных ресурсах.
- **Vision (изображения, PDF-страницы)**: `llava` — мультимодальный промт объединяет язык ответа и очищенный вопрос; при наличии контекст документа добавляется перед вопросом. Выбор: минимальные требования к памяти в оффлайн-среде.
- **Ограничения**: контекст и скорость зависят от CPU/GPU пилота; для больших документов требуется нарезка и батчинг; качество ниже, чем у тяжёлых моделей.
- **Использование в пайплайнах**: QA/RAG по артефактам, уточнение таблиц/смет, извлечение текста из PDF-страниц/изображений.
