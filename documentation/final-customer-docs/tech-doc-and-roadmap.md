...
#### 11 Используемые ИИ модели (Ollama)
- Назначение: локальный оффлайн-стек Ollama без исходящего трафика; выбраны компактные модели, которые поднимаются на пилотном стенде без отдельного GPU-кластера.
- Текущие:
  - **JSON / структурированные данные**: `deepseek-r1` (LLM). Отвечает по содержимому файлов (JSON/конвертированные ARP, GSFX, DXF/DWG, RTF, XLSX); язык ответа задаётся системной инструкцией. Выбор обусловлен хорошей устойчивостью к смешанным форматам при умеренных ресурсах.
  - **Vision (изображения, PDF-страницы)**: `llava` (мультимодальная). Работает с промптом, который объединяет язык ответа и очищенный вопрос; при наличии контекст документа добавляется перед вопросом. Выбор обусловлен минимальными требованиями к памяти в оффлайн-среде.
- Альтернативы (дают более высокое качество, но требуют существенно больше GPU/памяти):
  - **JSON / структурированные данные**: `llama-3.1-70b-instruct` или `qwen2.5-72b-instruct` — выше качество рассуждений и длинный контекст, поэтому точнее ответы по большим документам и сложным связям.
  - **Vision**: `llama-3.2-vision` или `florence-2-large` — лучше распознают мелкие элементы на PDF/чертежах и устойчивее к шуму, что повышает полноту извлечения данных.
