...
## 0.1 Критерии приёмки по областям
- Запросы и отчёты: ответ ≤ 30 сек; структура Excel/JSON/CSV соответствует шаблонам; trace на источники обязателен; ошибки/предупреждения возвращаются явно.
- OCR/штампы: CER/WER печатный текст ≤ 5–10%; полнота штампа ≥ 97% по полям; время OCR ≤ 30 сек/страница; детект типа PDF ≤ 3 сек/док.
- Таблицы (PDF/DXF): полнота извлечения ≥ 95%; заполненность ценовых полей ≥ 90%; сохранена структура (merged/иерархия/итоги); SLA в пределах 30 сек на документ/заданного лимита.
- Сметы (ARP/GSFX/XML/XLSX): полнота ключевых полей ≥ 95%; qty/unitPrice/amount валидны; round-trip корректен; статус формул (OK/Warning/Broken) зафиксирован; SLA в пределах согласованных лимитов по объёму.
- Нормативка: покрытие приоритетных норм ≥ 90% контрольного набора; отчёты о несоответствиях с точными ссылками (норматив/пункт/редакция/дата) за ≤ 30 сек; версии правил сохраняются.
- ACL и безопасность: 100% артефактов с projectId/rdCipher; отказ при нарушении ACL логируется; аудит действий администратора и выдачи привилегий; ретенция логов по согласованной политике.
- Инфраструктура и оборудование: конфигурация стенда соответствует согласованным минимальным/рекомендованным параметрам; сегментация сети, закрытые исходящие соединения; бэкапы/ретенция включены и проверены.
...
## 17.1 Приёмочные артефакты
- Техническое задание (текущий документ) с целевой архитектурой, планом и требованиями.
- Отчёт по анализу и выбору моделей (LLM/RAG, OCR/штамп, таблицы, сметы, нормативка) с критериями и результатами сравнения.
- Отдельные прототипы по задачам: OCR/штамп PDF; таблицы PDF/DXF; сметы ARP/GSFX/XML; нормативка/выжимки; RAG-ответы/trace.
- Результаты тестирования прототипов: метрики по контрольным наборам, артефакты (titleBlock.json, tables.json, estimate.json/CSV, validationReport.json, отчёты о несоответствиях, примеры Excel/CSV), логи выполнения.
- Контрольный набор данных и шаблонов (при условии предоставления заказчиком) для воспроизводимости приёмки.
